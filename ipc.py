...
def get_cnn_network_graphprot(rna_len = 501, nb_filter = 16):
    ...
    #model.add(Bidirectional(LSTM(10*nb_filter)))
...
def load_predict_graphprot_data():
    ...
    fw = open('result_file_graphwww', 'w')
    seq_hid = 16
    ...
    for protein in os.listdir(data_dir):
        ...
        print 'posi', label.count(1)
        print 'nega', label.count(0)
        '''
...
def run_rbp31():
    ...
    trids = get_6_trids()
    nn_dict = read_rna_dict()
    fw = open('result_file_rbp31_lstm', 'w')
    for protein in os.listdir(data_dir):
        print protein
        fw.write(protein + '\t')
